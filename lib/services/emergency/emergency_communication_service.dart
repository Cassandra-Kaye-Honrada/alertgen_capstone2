...
class EmergencyCommunicationService {
  ...
  // SIM card preference key
  static const String _simCardPreferenceKey = 'preferred_sim_card_slot';

  // Save the preferred SIM card slot (0 for SIM 1, 1 for SIM 2)
  Future<void> savePreferredSimCard(int simSlot) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt(_simCardPreferenceKey, simSlot);
      print('Preferred SIM card saved: SIM ${simSlot + 1}');
    } catch (e) {
      print('Error saving preferred SIM card: $e');
    }
  }

  // Get the preferred SIM card slot
  Future<int?> getPreferredSimCard() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final simSlot = prefs.getInt(_simCardPreferenceKey);
      print('Retrieved preferred SIM card: ${simSlot != null ? "SIM ${simSlot + 1}" : "None"}');
      return simSlot;
    } catch (e) {
      print('Error getting preferred SIM card: $e');
      return null;
    }
  }

  // Clear the saved SIM preference
  Future<void> clearPreferredSimCard() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.remove(_simCardPreferenceKey);
      print('Preferred SIM card cleared');
    } catch (e) {
      print('Error clearing preferred SIM card: $e');
    }
  }

  // Check if device has dual SIM capability
  Future<bool> isDualSimDevice() async {
    try {
      const platform = MethodChannel('emergency_sms');
      final bool isDualSim = await platform.invokeMethod('isDualSim');
      return isDualSim;
    } catch (e) {
      print('Error checking dual SIM capability: $e');
      return false;
    }
  }

  // Get available SIM cards info
  Future<List<Map<String, dynamic>>> getAvailableSimCards() async {
    try {
      const platform = MethodChannel('emergency_sms');
      final List<dynamic> simCards = await platform.invokeMethod('getSimCards');
      return simCards.map((sim) => Map<String, dynamic>.from(sim)).toList();
    } catch (e) {
  ...
  Future<bool> sendViaPlatformChannel(
    ...
    try {
      ...
      // Get preferred SIM card if available
      final simSlot = await getPreferredSimCard();

      final bool result = await platform.invokeMethod('sendDirectSMS', {
        'phoneNumber': phoneNumber,
        'message': message,
        'simSlot': simSlot, // Pass the preferred SIM slot
      });
  ...
  Future<void> makeDirectCall(
    ...
    try {
      ...
      // Get preferred SIM card
      final simSlot = await getPreferredSimCard();
      
      if (simSlot != null) {
        // Try to make call with specific SIM card
        bool? success = await makeCallWithSimCard(phoneNumber, simSlot);
        if (success == true) {
          print('Call made successfully with SIM ${simSlot + 1}');
          return;
        }
      }

      // Fallback to default calling method
      bool? canCall = await FlutterPhoneDirectCaller.callNumber(phoneNumber);
      if (canCall != true) {
        await makeCallViaApp(phoneNumber);
      }
  ...
  // Make call with specific SIM card
  Future<bool?> makeCallWithSimCard(String phoneNumber, int simSlot) async {
    try {
      const platform = MethodChannel('emergency_sms');
      final bool result = await platform.invokeMethod('makeCallWithSim', {
        'phoneNumber': phoneNumber,
        'simSlot': simSlot,
      });
      return result;
    } catch (e) {
      print('Error making call with SIM card: $e');
      return null;
    }
  }
